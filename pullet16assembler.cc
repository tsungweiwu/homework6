...
string Assembler::GetInvalidMessage(string leadingtext, string symbol) {
  has_an_error_ = true;
  string returnvalue = "***** ERROR: INVALID " + leadingtext + ": " + symbol;
  return returnvalue;
}
...
string Assembler::GetInvalidMessage(string leadingtext, Hex hex) {
  has_an_error_ = true;
  string returnvalue = "***** ERROR: HEX OPERAND INVALID: " + leadingtext;
  return returnvalue;
}
...
string Assembler::GetUndefinedMessage(string badtext) {
  has_an_error_ = true;
  string returnvalue = "***** ERROR: UNDEFINED SYMBOL: " + badtext;
  return returnvalue;
}
...
void Assembler::PassOne(Scanner& in_scanner) {
    ...
    if (line.substr(0, 1) == "*") {
      ...
      if (line.substr(0, 3) != "   ") {  // reading label
        ...
        if (symboltable_.count(label) == 0) {
          ...
          if ((int)label[0] < 64 || (int)label[0] > 91) {
             codeline.SetErrorMessages(GetInvalidMessage("SYMBOL", label));
             Utils::log_stream << GetInvalidMessage("SYMBOL", label) << endl;
          }
        } else {
          // sets a flag for duplicate labels
          symboltable_.at(label).SetMultiply();
          has_an_error_ = true;
          string err = "***** ERROR: SYMBOL " + label + " IS MULTIPLY DEFINED:";
      ...
      if (mnemonic == "ORG") {
        if (jumpvalue > 4095 || jumpvalue < 0) {
          Utils::log_stream << "***** ERROR Invalid ORG at line: " << linecounter_ << endl;
          has_an_error_ = true;
        } else {
        ...
        if ( (jumpvalue + pc_in_assembler_) > 4095 || jumpvalue < 1) {
          Utils::log_stream << "***** ERROR: Invalid DS at line: " << linecounter_ << endl;
          has_an_error_ = true;
        } else {
        pc_in_assembler_ += jumpvalue;
        }
  ...
  if (found_end_statement_ == 0) {
    Utils::log_stream << "***** ERROR: NO END STATEMENT:" << endl;
    has_an_error_ = true;
  }
...
void Assembler::PassTwo() {
  ...
  Utils::log_stream << "Memory size: " << memory_.size() << endl;
  string bitstring = "";
  Symbol symbol;
  ...
  for (auto it = codelines_.begin(); it != codelines_.end(); it++) {
    ...
    if (op_it != opcodes.end()) {
      ...
      if (sym_it != symboltable_.end()) {
        ...
        string err = GetUndefinedMessage("");
        (*it).SetErrorMessages("***** ERROR: SYMBOL UNDEFINED:");
...
#endif
  Utils::log_stream << "PCL Memory size: " << memory_.size() << endl;
  string s = "";
